...
Handles different file formats: .jpg, .png, .tif
"""
...
# Try to import openslide for .tif support
try:
    import openslide
    OPENSLIDE_AVAILABLE = True
except ImportError:
    OPENSLIDE_AVAILABLE = False
    warnings.warn("OpenSlide not available. .tif files will be read with PIL/cv2")
...
def read_image(image_path: str, level: int = 0) -> np.ndarray:
    """
    Read image from various formats.
    
    Args:
        image_path: Path to image file
        level: Level for multi-resolution images (0 = highest resolution)
        
    Returns:
        Image as numpy array (RGB)
    """
    if image_path.endswith('.tif') and OPENSLIDE_AVAILABLE:
        # Use OpenSlide for .tif files
        try:
            ...
            logging.warning(f"OpenSlide failed for {image_path}: {e}. Falling back to PIL.")
    
    # Fallback to PIL/cv2 for other formats or if OpenSlide fails
...
def create_split_file(
    ...
    for meta in patch_metadata:
        ...
        if image_id in train_set:
            ...
            logger.error(f"Image {image_id} not found in any split!")
            continue
...
def main():
    ...
    if not OPENSLIDE_AVAILABLE:
        logger.warning(
            "OpenSlide not available. .tif files will be read with PIL. "
            "For better performance with .tif files, install openslide-python."
        )
    ...
    for dataset in datasets:
        ...
        if not os.path.exists(wsi_dir):
            logger.warning(f"WSI directory not found: {wsi_dir}")
            continue
            
        if not os.path.exists(mask_dir):
            logger.warning(f"Mask directory not found: {mask_dir}. Run create_binary_masks.py first.")
            continue
        
        # Get WSI files
        wsi_extensions = ['.jpg', '.jpeg', '.png', '.tif', '.tiff']
        wsi_files = []
        for ext in wsi_extensions:
            wsi_files.extend([f for f in os.listdir(wsi_dir) if f.lower().endswith(ext)])
        wsi_files = sorted(wsi_files)
        
        # Get mask files (should all be .png after binary mask creation)
        mask_files = sorted([f for f in os.listdir(mask_dir) if f.endswith('.png')])
        
        # Match files by stem name
        paired_files = []
        for wsi_file in wsi_files:
            wsi_stem = Path(wsi_file).stem
            # Find corresponding mask
            mask_file = None
            for mf in mask_files:
                if Path(mf).stem == wsi_stem:
                    mask_file = mf
                    break
