"""
Universal training script for epidermis segmentation.
...
def create_model(architecture: str, encoder: str, config: Optional[Dict] = None):
    """
    Create model based on architecture and encoder.
    
    Args:
        architecture: Model architecture ('unet' or 'deeplabv3plus')
        encoder: Encoder name ('resnet50', 'efficientnet-b3')
    ...
    if architecture == 'unet':
        ...
        return models[f'unet_{encoder}']
    elif architecture == 'deeplabv3plus':
        models = create_deeplabv3plus_models(config)
        return models[f'deeplabv3plus_{encoder}']
    else:
...
def main():
    ...
    experiment_name = args.experiment_name or f"{args.architecture}_{args.encoder}_{timestamp}"
    ...
    # Create model
    logger.info(f"Creating {args.architecture.upper()} model with {args.encoder} encoder...")
    model = create_model(args.architecture, args.encoder, config.get('model', {}))
